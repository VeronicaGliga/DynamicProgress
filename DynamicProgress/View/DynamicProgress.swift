...
class DynamicProgress: NSObject, ObservableObject {
    ...
    func updateProgressView(to: CGFloat) {
        ...
        NotificationCenter.default.post(name: NSNotification.Name("UPDATE_PROGRESS"), 
                                        object: nil,
                                        userInfo: ["progress": to])
    }
    ...
    func removeProgressWithAnimations() {
        NotificationCenter.default.post(name: NSNotification.Name("CLOSE_PROGRESS_VIEW"), 
                                        object: nil)
    }
...
struct DynamicProgressView: View {
            ...
            ctx.drawLayer { context in
                for index in [1,2] {
                    if let resolvedImage = ctx.resolveSymbol(id: index) {
                        // Dynamic Area Top Offset = 11
                        // Capsule Height = 37 / 2 -> 18
                        context.draw(resolvedImage, 
                                     at: CGPoint(x: size.width / 2, y: 11 + 18))
                    }
                }
            }
        } symbols: {
            ...
            if let info = output.userInfo,
               ...
               progress < 1.0 {
                ...
                if (progress * 100).rounded() == 100.0 {
                    ...
                    // Hiding Status Bar When Half AlertView is Opened
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                        progressBar.hideStatusBar = true
                    }
                    
                    // After 2-3 Sec's Closing Alert and Removing View from Root Controller
    ...
    func CustomAlertView() -> some View {
        ...
        // Delay For Circle Animation to Finish
        .animation(.interactiveSpring(response: 0.5, 
                                      dampingFraction: 0.7,
                                      blendDuration: 0.7).delay(showAlertView ? 0.35 : 0),
                   value: showAlertView)
    }
        ...
        ZStack{
            ...
            ZStack{
                ...
                Circle()
                    ...
                    .stroke(config.tint, style: StrokeStyle(lineWidth: 4, 
                                                            lineCap: .round,
                                                            lineJoin: .round))
